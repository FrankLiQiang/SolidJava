...
public class RingActivity extends Activity {
    ...
    private void createEllipse() {
        ...
        for (int i = 0; i < parallelNum; i++) {
            ...
            for (int j = 0; j < ellipsePointNum; j++) {
                ellipseGroup[i].ellipse[j].x =
                        Common.ObjCenter.x - R0 * (float) Math.cos(2 * Common.PI / ellipsePointNum * j);
                ellipseGroup[i].ellipse[j].y = Common.ObjCenter.y - r * (float) Math.sin(2 * Common.PI / parallelNum * i);
                ellipseGroup[i].ellipse[j].z =
                        Common.ObjCenter.z + R0 * (float) Math.sin(2 * Common.PI / ellipsePointNum * j);
            }
        }
        ...
        for (int i = parallelNum; i < parallelNum + meridianNum; i++) {
            ...
            for (int j = 0; j < ellipsePointNum; j++) {
                R0 = R - r * (float)Math.cos(2 * Common.PI / ellipsePointNum * j);
                ellipseGroup[i].ellipse[j].x =
                        Common.ObjCenter.x - R0 * (float) Math.cos(2 * Common.PI / meridianNum * i);
                ellipseGroup[i].ellipse[j].y = Common.ObjCenter.y - r * (float) Math.sin(2 * Common.PI / ellipsePointNum * j);
                ellipseGroup[i].ellipse[j].z =
                        Common.ObjCenter.z + R0 * (float) Math.sin(2 * Common.PI / meridianNum * i) ;
            }
        }
